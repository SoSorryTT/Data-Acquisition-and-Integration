...
def get_all_year_basin_annual_rainfall(basin_id):
    with db_cursor() as cs:
        cs.execute("""
        SELECT year, SUM(amount) AS amount
        FROM basin
        INNER JOIN station ON basin.basin_id = station.basin_id
        INNER JOIN rainfall ON station.station_id = rainfall.station_id
        WHERE basin.basin_id = %s
        GROUP BY year
    """, [basin_id])
    result = [models.AnnualRainfall(year, amount) for year, amount in cs.fetchall()]
    return result
