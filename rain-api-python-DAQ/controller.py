...
def get_station_details(station_id):
    ...
    if result:
        ...
        return models.StationFull(station_id=station_id, basin_id=basin_id, name=ename, lat=lat, lon=lon)
    else:
        abort(404)

def get_basin_year_details(basin_id, year):
    with db.cursor() as cs:
        cs.execute("""
    ...
    if result:
        rainfall = result
        return models.BasinYear(basin_id=basin_id, rainfall=rainfall, year=year)
    else:
        abort(404)
